...
const (
	...
	generatedTemplHeader      = "// Code generated by cmd/approutegen from internal/web/app. DO NOT EDIT."
	generatedServerFileName   = "server_gen.go"
...
func Run(cfg Config) error {
	paths := cfg.Layout
	if strings.TrimSpace(paths.AppDir) == "" {
		return errors.New("project layout app dir is required")
	}
	if strings.TrimSpace(paths.GeneratedDir) == "" {
		return errors.New("project layout generated dir is required")
	}
	if strings.TrimSpace(paths.ResolverDir) == "" {
		return errors.New("project layout resolver dir is required")
	}

	routes, err := discoverRouteFiles(paths.AppDir, paths.GeneratedDir)
	if err != nil {
		return err
	}
	...
	if err := os.MkdirAll(paths.ResolverDir, 0o755); err != nil {
		return fmt.Errorf("create resolver namespace root %q: %w", paths.ResolverDir, err)
	}
	resolverFilePath := filepath.Join(paths.ResolverDir, generatedResolverFileName)
	...
	serverSource, err := generateServerSource(paths)
	if err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(paths.GeneratedDir, generatedServerFileName), serverSource, 0o644); err != nil {
		return fmt.Errorf("write %s: %w", generatedServerFileName, err)
	}
...
func resolverImportPath(paths bundler.ProjectLayout) string {
	relativePath := strings.TrimSpace(paths.ResolverImport)
	if relativePath == "" {
		relativePath = "internal/web/resolvers"
	}
	return appImportPath(paths.AppModulePath, relativePath)
}

func runtimeImportPath(paths bundler.ProjectLayout) string {
	relativePath := strings.TrimSpace(paths.RuntimeImport)
	if relativePath == "" {
		relativePath = "internal/web/runtime"
	}
	return appImportPath(paths.AppModulePath, relativePath)
}
...
func buildRouteMetas(pages []templateDef, paths bundler.ProjectLayout) ([]routeMeta, error) {
	_ = paths
...
func parsePageViewType(pageTemplatePath string) (string, error) {
	...
	if !strings.HasPrefix(viewType, runtimePackageName+".") {
		return "", fmt.Errorf(
...
func validateLayoutTemplateSignature(layout templateDef) error {
	...
	viewType := strings.TrimSpace(matches[1])
	if viewType != runtimePackageName+".RootLayoutView" {
		return fmt.Errorf(
...
func generateResolverNamespaceSource(
	...
	buffer.WriteString("\t\"" + runtimeImportPath(paths) + "\"\n")
...
func generateRegistrySource(
	...
	importLines := []string{
		...
		fmt.Sprintf("route_resolvers %q", resolverImportPath(paths)),
...
func generateServerSource(paths bundler.ProjectLayout) ([]byte, error) {
	importLines := []string{
		"\"fmt\"",
		"\"net/http\"",
		"\"strings\"",
	...
	if paths.ServerFeatures.StaticAssets {
		importLines = append(
			...
			"\"os\"",
			"\"path/filepath\"",
			fmt.Sprintf("%q", frameworkModulePath+"/framework/staticassets"),
		)
	}
	...
	buffer.WriteString("\tif err := MountRoutes(mux, cfg); err != nil {\n")
	buffer.WriteString("\t\treturn nil, err\n")
	buffer.WriteString("\t}\n")
	buffer.WriteString("\treturn mux, nil\n")
	buffer.WriteString("}\n\n")

	buffer.WriteString("func MountRoutes(mux *http.ServeMux, cfg ServerConfig) error {\n")
	...
	if paths.ServerFeatures.StaticAssets {
		...
		buffer.WriteString("\t\truntimeCfg.StaticAssetBasePath = staticMount.URLPrefix\n")
		buffer.WriteString("\t}\n")
	} else {
		buffer.WriteString("\tstaticMount := httpserver.StaticMount{}\n")
	}
	...
	if paths.ServerFeatures.I18nRouting {
		...
		buffer.WriteString("\tif err != nil {\n")
		buffer.WriteString("\t\treturn fmt.Errorf(\"create i18n resolver: %w\", err)\n")
		buffer.WriteString("\t}\n")
		buffer.WriteString("\tbypassPrefixes := []string{}\n")
		buffer.WriteString("\tif strings.TrimSpace(staticMount.URLPrefix) != \"\" {\n")
	...
	if paths.ServerFeatures.PublicFiles {
		...
		buffer.WriteString("\t\tif err != nil {\n")
		buffer.WriteString("\t\t\treturn fmt.Errorf(\"build public files middleware: %w\", err)\n")
		buffer.WriteString("\t\t}\n")
		buffer.WriteString("\t\thandler = publicMiddleware(handler)\n")
		buffer.WriteString("\t}\n")
	}
	...
	if paths.ServerFeatures.StaticAssets {
		...
		buffer.WriteString("\t}\n")
		buffer.WriteString("\tmanifest, err := staticassets.ReadManifest(trimmedManifestPath)\n")
		buffer.WriteString("\tif err != nil {\n")
		buffer.WriteString(
			"\t\treturn httpserver.StaticMount{}, " +
				"fmt.Errorf(\"load static manifest %q: %w\", trimmedManifestPath, err)\n",
		)
		buffer.WriteString("\t}\n")
		buffer.WriteString("\tstaticDir := filepath.Clean(filepath.Dir(trimmedManifestPath))\n")
		buffer.WriteString("\tinfo, statErr := os.Stat(staticDir)\n")
		buffer.WriteString("\tif statErr != nil {\n")
		buffer.WriteString(
			"\t\treturn httpserver.StaticMount{}, " +
				"fmt.Errorf(\"stat static build dir %q: %w\", staticDir, statErr)\n",
		)
		buffer.WriteString("\t}\n")
		buffer.WriteString("\tif !info.IsDir() {\n")
		buffer.WriteString(
			"\t\treturn httpserver.StaticMount{}, " +
	...
	formatted, err := format.Source(buffer.Bytes())
	if err != nil {
		return nil, fmt.Errorf("format server source: %w", err)
	}
	return formatted, nil
}
