...
func stylesheetHelperSource(layout projectlayout.ProjectLayout, outputPath string) ([]byte, error) {
	...
	buffer.WriteString("\tif err := run(); err != nil {\n")
	buffer.WriteString("\t\tfmt.Fprintln(os.Stderr, err)\n")
	buffer.WriteString("\t\tos.Exit(1)\n")
	buffer.WriteString("\t}\n")
	buffer.WriteString("}\n\n")
	buffer.WriteString("func run() error {\n")
	...
	buffer.WriteString("\t}\n")
	buffer.WriteString("\tif builder.Len() == 0 {\n")
	...
	buffer.WriteString("\t\treturn fmt.Errorf(\"create stylesheet dir: %w\", err)\n")
	buffer.WriteString("\t}\n")
	buffer.WriteString("\tif err := os.WriteFile(outputPath, []byte(builder.String()), 0o644); err != nil {\n")
	buffer.WriteString("\t\treturn fmt.Errorf(\"write stylesheet: %w\", err)\n")
	buffer.WriteString("\t}\n")
	buffer.WriteString("\treturn nil\n")
	buffer.WriteString("}\n")
