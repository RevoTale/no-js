...
import (
	...
	gen "example.com/no-js-e2e/namespacedtemplcssapp/web/generated"
...
func main() {
	...
	dashboard := request(handler, http.MethodGet, "/dashboard", nil)
	stylesheetURL := stylesheetURLFromBody(dashboard.Body)

	out := report{
		Dashboard: dashboard,
		Partial: request(handler, http.MethodGet, "/dashboard", map[string]string{
			"HX-Request": "true",
		}),
		NotFound:      request(handler, http.MethodGet, "/unknown", nil),
