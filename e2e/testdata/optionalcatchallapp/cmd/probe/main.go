...
import (
	...
	gen "example.com/no-js-e2e/optionalcatchallapp/web/generated"
...
type report struct {
	Root          responseSnapshot `json:"root"`
...
func main() {
	...
	root := request(handler, http.MethodGet, "/library", nil)
	stylesheetURL := stylesheetURLFromBody(root.Body)

	out := report{
		Root:          root,
		Nested:        request(handler, http.MethodGet, "/library/a/b", nil),
		Partial:       request(handler, http.MethodGet, "/library", map[string]string{"HX-Request": "true"}),
